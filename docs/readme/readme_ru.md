...
Также реализована кеширование байт-кода, позволяющая переиспользовать однажды сгенерированный класс. Это крайне полезно
в тех случаях, когда собран финальный (и, что важно, неизменный в будущем) jar-файл, который будет опубликован на
продакшен:
...
Этот код после первого запуска создаст директорию cache, где будет хранить сгенерированный байт-код. При следующих
запусках он вместо генерации будет сразу загружать его. При удалении кэша генерация произойдет заново.
